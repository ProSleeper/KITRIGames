...
void WorldAxis::Setup(void)
{
	...
	for (int i = 0; i < 18; ++i)
		m_arrVertexsY[i].color = D3DXCOLOR(0, 1, 0, 1);
	...
	for (int i = 0; i < 18; ++i)
	{
		...
		m_arrVertexsX[i].color = D3DXCOLOR(1, 0, 0, 1);
	}
	...
	for (int i = 0; i < 18; ++i)
	{
		...
		m_arrVertexsZ[i].color = D3DXCOLOR(0, 0, 1, 1);
	}
}

void WorldAxis::Update(void)
{
}
...
void WorldAxis::Render(void)
{
	...
	DEVICE->SetFVF(static_cast<DWORD>(D3DFVF_XYZ_COLOR::FORMAT::FVF));					// 포맷형식
