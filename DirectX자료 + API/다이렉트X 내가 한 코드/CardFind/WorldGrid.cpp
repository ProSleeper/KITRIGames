...
void WorldGrid::Setup( void )
{
	...
	for( int i = 0; i < iCountX; i++ )
	{
		...
		// 색상
		m_pFVFVertexs[idx].color = (i % m_iTileInterval == 0) ?	D3DXCOLOR(0, 0, 0, 1) : D3DXCOLOR(0.3f, 0.3f, 0.3f, 1);
		m_pFVFVertexs[idx + 1].color = (i % m_iTileInterval == 0) ? D3DXCOLOR(0, 0, 0, 1) : D3DXCOLOR(0.3f, 0.3f, 0.3f, 1);
